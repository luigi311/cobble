...
impl CallMap {
    ...
    async fn remove_by_path(&self, call_path: &str) -> Option<u32> {
        ...
        let cookie = map
            .iter()
            .find(|(_, c)| c.call_path == call_path)
            .map(|(k, _)| *k);
...
pub async fn run_call_monitor(
    daemon: CobbleDaemon,
    mut action_rx: mpsc::UnboundedReceiver<(String, u32)>,
) {
    ...
    let conn = match Connection::system().await {
        ...
        Err(e) => {
            warn!("call-monitor: cannot connect to system bus: {e}");
            return;
        }
    };
    let map = CallMap::new();
...
async fn handle_mm_call(conn: &Connection, daemon: &CobbleDaemon, map: &CallMap, call_path: &str) {
    ...
    map.insert(
        cookie,
        ModemCall {
            bus_name: "org.freedesktop.ModemManager1".into(),
    ...
    let _ = daemon
        .push_incoming_call(cookie, number.clone(), number)
        .await;
}

async fn handle_mm_property_change(
    _conn: &Connection,
    daemon: &CobbleDaemon,
    map: &CallMap,
    msg: &zbus::Message,
...
async fn watch_ofono(conn: &Connection, daemon: &CobbleDaemon, map: &CallMap) {
    ...
    match list_mm_objects(conn, "org.ofono", "/org/ofono").await {
        Ok(modems) => {
            trace!(
                "call-monitor: found {} oFono modems: {modems:?}",
                modems.len()
            );
            for modem in modems {
    ...
    while let Some(msg) = stream.next().await {
        ...
        {
            let body = msg.body();
            if let Ok((call_path, props)) =
                body.deserialize::<(zbus::zvariant::ObjectPath<'_>, HashMap<String, OwnedValue>)>()
            {
...
async fn scan_ofono_calls(conn: &Connection, daemon: &CobbleDaemon, map: &CallMap, modem: &str) {
    let calls: Option<Vec<zbus::zvariant::ObjectPath<'_>>> =
        get_ofono_prop(conn, modem, "org.ofono.VoiceCallManager", "Calls").await;
    if let Some(calls) = calls {
        for call_path in calls {
            let path = call_path.to_string();
            let state: Option<String> =
                get_ofono_prop(conn, &path, "org.ofono.VoiceCall", "State").await;
            if let Some(ref s) = state
                && (s == "incoming" || s == "waiting")
            {
                let number: Option<String> =
                    get_ofono_prop(conn, &path, "org.ofono.VoiceCall", "LineIdentification").await;
...
async fn handle_ofono_property_change(daemon: &CobbleDaemon, map: &CallMap, msg: &zbus::Message) {
    ...
    let body = msg.body();
    let Ok((prop_name, value)) = body.deserialize::<(String, OwnedValue)>() else {
        return;
    };
    if prop_name != "State" {
        return;
...
async fn handle_watch_action(
    ...
    debug!(
        "call-monitor: {action} → {}/{}",
        call.bus_name, call.call_path
    );
    let _ = conn
        .call_method(
            Some(call.bus_name.as_str()),
            call.call_path.as_str(),
            Some(iface),
            method,
            &(),
        )
...
async fn list_mm_objects(
    ...
    for line in xml.lines() {
            ...
            && let Some(end) = name.find('"')
        {
            let child = &name[..end];
            if child != "Voice" {
                paths.push(format!("{base_path}/{child}"));
            }
        }
    }
    Ok(paths)
...
{
    let reply = conn
        .call_method(Some("org.ofono"), path, Some(iface), "GetProperties", &())
        .await
        .ok()?;
